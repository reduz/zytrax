...
#include "list.h"
//This code was adapted from Ardour
...
struct VSTState {
	...
	VSTState *next;
	Window linux_window;
	Window linux_plugin_ui_window;
	AudioEffectVST2 *effect_vst;
	EffectEditorVST2 *editor_vst;
	int width, height;
		...
		case ConfigureNotify: {
			...
			if (window == (Window)(vstfx->linux_window)) {
				...
				if (width != vstfx->width || height != vstfx->height) {
					...
					//vstfx->window_size_changed_or_something();
				}
			...
			for (vstfx = vstfx_first; vstfx; vstfx = vstfx->next) {
				...
				if (vstfx->destroy) {
					if (vstfx->linux_window) {
						vstfx->effect_vst->close_user_interface();
				...
				if (vstfx->linux_window == 0) {
					if (vstfx_create_editor(vstfx)) {
						printf("** ERROR ** VSTFX : Cannot create editor for plugin %s\n", vstfx->effect_vst->get_name().utf8().get_data());
						vstfx_event_loop_remove_plugin(vstfx);
						pthread_cond_signal(&vstfx->window_status_change);
						pthread_mutex_unlock(&vstfx->lock);
						goto again;
				...
				vstfx->effect_vst->process_user_interface(); //->dispatcher(vstfx->plugin, effEditIdle, 0, 0, NULL, 0);
...
int vstfx_init() {
	...
	if (LXVST_XDisplay == NULL) {
		printf("** ERROR ** VSTFX: Failed opening connection to X\n");
	...
	if (thread_create_result != 0) {
		...
		printf("** ERROR ** VSTFX: Failed starting GUI event thread\n");
...
int vstfx_run_editor(AudioEffectVST2 *p_effect, EffectEditorVST2 *p_editor) {
	...
	vstfx->effect_vst = p_effect;
	vstfx->editor_vst = p_editor;
	pthread_mutex_init(&vstfx->lock, 0);
	//pthread_mutex_init (&vstfx->vstfx_lock, 0);
...
int vstfx_create_editor(VSTState *vstfx) {
	...
	if (!(vstfx->effect_vst->has_user_interface())) {
		printf("** ERROR ** VSTFX: Plugin \"%s\" has no editor\n", vstfx->effect_vst->get_name().utf8().get_data());
		return -1;
	}
